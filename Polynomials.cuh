...
// --- Second order
template <class T> inline __host__ __device__ T Legendre2(const T& x) { return ((static_cast<T>(3.0) * x * x) - static_cast<T>(1.0)) * static_cast<T>(0.5f); }
...
/**************************************************************/
/* FACTORIZED LEGENDRE POLYNOMIALS CALCULATION FUNCTION - GPU */
/**************************************************************/
...
/**************************************************************/
/* FACTORIZED LEGENDRE POLYNOMIALS CALCULATION FUNCTION - CPU */
/**************************************************************/
template <class T>
thrust::pair<thrust::pair<T *, T*>, T*> h_generateLegendreFactorized(const int maxDegreeX, const int maxDegreeY, const int M_x, const int M_y) {

	// --- Generating the (csi, eta) grid
	T *h_csi = h_linspace(-static_cast<T>(1), static_cast<T>(1), M_x);
	T *h_eta = h_linspace(-static_cast<T>(1), static_cast<T>(1), M_y);

	thrust::pair<T *, T *> h_CSI_ETA = h_meshgrid(h_csi, M_x, h_eta, M_y);
	T *h_CSI = h_CSI_ETA.first;
	T *h_ETA = h_CSI_ETA.second;

	// --- Generating the Legendre polynomials
	const int N = M_x * M_y;
	
	T *h_Leg = (T *)malloc(maxDegreeX * maxDegreeY * N * sizeof(T));
	
	for (int k = 0; k < maxDegreeY; k++)
		for (int j = 0; j < maxDegreeX; j++)
			for (int i = 0; i < N; i++)
				h_Leg[(k * maxDegreeX + j) * N + i] = LegendreN(k, h_CSI[i]) * LegendreN(j, h_ETA[i]);

	free(h_csi);
	free(h_eta);

	return thrust::make_pair(thrust::make_pair(h_CSI, h_ETA), h_Leg);

}

template thrust::pair<thrust::pair<float  *, float *>, float *>  h_generateLegendreFactorized<float> (const int, const int, const int, const int);
template thrust::pair<thrust::pair<double *, double*>, double*>  h_generateLegendreFactorized<double>(const int, const int, const int, const int);
...
/********************************************************************************/
/* ZERNIKE POLYNOMIALS CALCULATION FUNCTION - SINGLE INDEX - (x, y) INPUT - GPU */
/********************************************************************************/
...
/********************************************************************************/
/* ZERNIKE POLYNOMIALS CALCULATION FUNCTION - SINGLE INDEX - (x, y) INPUT - CPU */
/********************************************************************************/

// --- Zernike polynomials with single index.
template <class T>
T * h_generateZernikep(const T * __restrict__ h_CSI, const T * __restrict__ h_ETA, const int maxDegree, const int M_x, const int M_y) {

	thrust::pair<T *, T *> h_RHO_THETA = h_Cartesian2Polar(h_CSI, h_ETA, M_x * M_y, static_cast<T>(1) / sqrt(static_cast<T>(2)));
	T *h_RHO	= h_RHO_THETA.first;
	T *h_THETA  = h_RHO_THETA.second;

	const int N = M_x * M_y;
	
	T *h_Zernike = (T *)malloc(N * maxDegree * sizeof(T));
	
	for (int j = 0; j < maxDegree; j++)
		for (int i = 0; i < N; i++)
			h_Zernike[j * N + i] = Zernikep(j + 1, h_RHO[i], h_THETA[i]);

	return h_Zernike;
}

template float  * h_generateZernikep<float> (const float  * __restrict__, const float  * __restrict__, const int, const int, const int);
template double * h_generateZernikep<double>(const double * __restrict__, const double * __restrict__, const int, const int, const int);
